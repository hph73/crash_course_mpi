...
// Example 2.1: send and receive arrays
int main(int argc, char** argv) {
    int my_PE_num;
    int array1[2][2] = {{1, 2}, {0, 0}};
    int array2[2][2] = {{0, 0}, {3, 4}};
    MPI_Status status;

    MPI_Init(&argc, &argv);
    MPI_Comm_rank(MPI_COMM_WORLD, &my_PE_num);
    if (my_PE_num == 0) {
        MPI_Send(array1[0], 2, MPI_INT, 1, 10, MPI_COMM_WORLD);
        MPI_Recv(array1[1], 2, MPI_INT, 1, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
        std::cout << "array1=" << std::endl;
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++)
                std::cout << array1[i][j] << std::endl;
    } else {
        MPI_Send(array2[1], 2, MPI_INT, 0, 10, MPI_COMM_WORLD);
        MPI_Recv(array2[0], 2, MPI_INT, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
